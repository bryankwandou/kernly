...
export function score(blocks, query) {
    ...
    const perBlock = live.map((b) => {
        ...
        for (const u of new Set(t))
            df.set(u, (df.get(u) || 0) + 1);
        return t;
    });
    const N = live.length;
