 ...
 */
const PROTECT = /(`[^`]*`|"[^"]*"|'[^']*'|\{[^}]*\}|\[[^\]]*\]|https?:\/\/\S+|[\w.-]+@[\w.-]+|\b[A-Z][A-Z0-9_]{2,}\b|\b\w+\(\)|\b\d[\d.,]*\b)/g;
const OPEN = String.fromCharCode(1);
const CLOSE = String.fromCharCode(2);
export function compact(text, strength) {
    if (strength <= 0)
    ...
    if (strength >= 0.5)
        work = work.replace(DETERMINERS, "");
    if (strength >= 0.75)
        work = work.replace(COPULAS, "");
    if (strength >= 0.9)
        work = work.replace(AUX, "");
    work = work
        .replace(/[ \t]{2,}/g, " ")
        .replace(/ +([.,;:!?])/g, "$1")
        .replace(/^[ \t,;]+/gm, "")
        .replace(/\n{3,}/g, "\n\n")
        .trim();
    work = work.replace(new RegExp(OPEN + "(\\d+)" + CLOSE, "g"), (_, i) => vault[Number(i)]);
